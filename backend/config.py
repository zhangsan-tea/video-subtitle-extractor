...
# 是否启用密集文字群过滤：当一帧中多个文字框在垂直方向密集排列时，视为叠加面板文字并过滤
DENSE_TEXT_FILTER = True

# 密集群最小文字框数量：连续 >= 此数量的密集排列文字框才被视为面板文字
# 建议值3-5，值越大越宽松（需要更多行才会被过滤）
DENSE_MIN_CLUSTER_SIZE = 3

# 垂直间距比值阈值：相邻文字框间距 < 文字高度 * 此值时视为密集排列
# 值越大越容易判定为密集，建议范围 1.5-3.0
DENSE_VERTICAL_GAP_RATIO = 2.0
